...
use  "$base_path_wd\download\input\IBS_final_panel.dta", clear
keep firm_id year 
keep if year < 1998
bys firm_id (year): egen min_year = min(year)
keep if year == min_year 
* among these old firms, keep only palm oil mills. 
merge 1:m firm_id using "$base_path_wd\download\output\IBS_PO_98_15.dta", generate(merge_mill_id) keepusing(firm_id year)
...
append using "$base_path_wd\download\output\IBS_PO_98_15.dta"
sort firm_id year
...
merge m:1 year using "$base_path_wd\build\input\wpi_palmoil_1998_2015.dta", nogenerate 
...
merge m:1 year_prov_distr using "$base_path_wd\build\input\mill_geolocalization\province_district_code_names_93_2016.dta", generate( /// 
...
** Add names
merge m:1 year_desa_id using  "$base_path_wd\build\input\mill_geolocalization\desa_code_names_98_2014.dta", generate( /// 
...
save "$base_path_wd\build\output\IBS_PO_98_15_cleaned.dta", replace
