# This script uses parcel dataframes built in prepare_lucfp.R
# These data frames are 2001 - 2018 panels of parcels of a given size (3x3km only for now), 
...
# Every datafreames have information on different measures of forest conversion, for each parcel in each year. 
...
parcel_set_w_average <- function(parcel_size, catchment_radius){
  ...
  parcels_centro <- readRDS(paste0("./outcome_variables/dataframes/panel_Indonesia_",parcel_size/1000,"km_",catchment_radius/1000,"CR.rds"))
  ...
  # 
  # 
  parallel_w_average <- function(ncores){
    ...
    annual_w_average <- function(t){
      ...
      parcels <- parcels_saved
      
     
      
      ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 
      # test apply rather than loop # 
      tic()
      ...
      for(voi in variables){
        ...
        parcels[s,paste0("wa_", voi)] <- sapply(parcels$reachable[s], 
                                                ...
                                                %>% is.na()
                                                %>% all() 
                                                %>% ifelse(yes = NA, no = sum(st_drop_geometry(x[,paste0("w_var_",voi)]),na.rm = T))) %>% length()
        ...
        saveRDS(parcels, here(paste0("build/input/temp_cs_wa_explanatory/cs_wa_explanatory_",
                                     parcel_size/1000,"km_",catchment_radius/1000,"CR",years[t],".rds")))
        toc()
      ...
      toc() # 7845 s. for distance and w columns. 
      # Define the variables of interest we want to compute the weighted averages of. 
      #variables <- c("cpo_price_imp1","cpo_price_imp2", "prex_cpo_imp1", "prex_cpo_imp2")
      variables <- c("min_year", "est_year", "est_year_imp", "max_year",
                     "ffb_price_imp1", "ffb_price_imp2", "in_ton_ffb_imp1", "in_ton_ffb_imp2", "in_val_ffb_imp1", "in_val_ffb_imp2",
                     "cpo_price_imp1","cpo_price_imp2", "out_ton_cpo_imp1", "out_ton_cpo_imp2", "out_val_cpo_imp1", "out_val_cpo_imp2",
                     "prex_cpo_imp1", "prex_cpo_imp2",
                     "pko_price_imp1","pko_price_imp2", "out_ton_pko_imp1", "out_ton_pko_imp2", "out_val_pko_imp1", "out_val_pko_imp2",
    ...
    foreach(t = 1:length(years), 
            ...
            .export = c("annual_w_average", "parcels_centro", "ibs_cs", "years", "catchment_radius", "indonesian_crs"), 
            .packages = c("sf", "tidyverse")) %dopar% 
      annual_w_average(t) # the function that is parallelly applied to different years. 
  }
  
  ### Execute it
  parallel_w_average(detectCores() - 1)
  wide_parcels <- list.files(path = here("build/input/temp_cs_wa_explanatory/"), 
                             pattern = paste0("cs_wa_explanatory_",parcel_size/1000,"km_",catchment_radius/1000,"CR"), 
                             full.names = TRUE) %>% readRDS() %>% cbind()
...
# This can be done outside of the annual_w_average function
