...
use "$base_path_wd\download\input\prepared_IO\IBS_outputs_prep.dta", clear

merge 1:1 firm_id year using "$base_path_wd\download\input\prepared_IO\IBS_inputs_prep.dta", generate(merge_codes_io) update
...
save "$base_path_wd\download\input\prepared_IO\IBSIO.dta", replace 
...
use "$base_path_wd\download\input\si_panel.dta", clear
...
merge 1:1 firm_id year using "$base_path_wd\download\input\IBS_final_panel.dta", generate(merge_fp) keepusing(export_dummy export_dummy_imp export_pct export_pct_imp /// 
	revenue_total pct_own_cent_gov_imp pct_own_loc_gov_imp pct_own_nat_priv_imp pct_own_for_imp workers_total_imp3)

merge 1:1 firm_id year using "$base_path_wd\download\input\prepared_IO\IBSIO.dta", generate(merge_io)
...
save "$base_path_wd\download\output\IBS_PO_98_15.dta", replace 
